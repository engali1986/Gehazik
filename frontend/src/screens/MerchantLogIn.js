...
const MerchantLogIn = ({ globalState, setGlobal }) => {
  ...
  const VarifyEmail = async () => {
    ...
    const Merchantvarified = await fetch("http://localhost:5000/LogInMerchant", {
      method: "POST",
    ...
    } else if (Merchantvarified.resp.email && Merchantvarified.resp.Merchantvarified === false) {
      Alert.current.classList.replace("alert-danger", "alert-success");
  ...
  const LogInSubmit = async () => {
    ...
    if (Credentials.Email.length > 0 && Credentials.Password.length > 0) {
      ...
      const UserLogIn = await fetch("http://localhost:5000/LogInMerchant", {
        method: "POST",
        body: JSON.stringify(Credentials),
        headers: {
          "Content-Type": "application/json",
      ...
      if (UserLogIn.resp) {
        ...
        if (typeof UserLogIn.resp === "object") {
          ...
          if (UserLogIn.resp.email && UserLogIn.resp.Merchantvarified === true) {
            ...
            Alert.current.innerText = "User Logged in successfully";
            Alert.current.style.maxHeight = "500px";
          } else if (UserLogIn.resp.email && UserLogIn.resp.Merchantvarified === false) {
            console.log(UserLogIn)
            Alert.current.classList.replace("alert-danger", "alert-success");
            Alert.current.innerText = "Varification code sent by email";
            Alert.current.style.maxHeight = "500px";
